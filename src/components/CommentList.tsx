...
const CommentList = ({comments, postId} : {comments:CommentWithUser[]; postId: number}) => {
    ...
    return (
        ...
        {user && <div className="flex items-center gap-4">
            ...
            <form action={add} className="flex-1 flex items-center justify-between bg-slate-100 rounded-xl text-sm px-4 py-4 w-full">
                <input type="text" placeholder="Escribe tu comentario" className="bg-transparent outline-none flex-1" onChange={e => setDesc(e.target.value)}/>
        ...
        <div className="">
            {/* COMMENT */}
            { optimisticComments.map((comment) => ( <div className="flex gap-4 justify-between mt-6" key={comment.id}>
                {/* AVATAR */}
