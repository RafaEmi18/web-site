...
const FriendRequestList = ({requests}:{requests:RequestWithUser[]}) => {
    ...
    const accept = async (requestId: number, userId: string) => {
        ...
        try{
            ...
            console.log(err)
            throw new Error("Algo Malo Sucedio!")
        }
    }
    ...
    return (
        <div className="">
        {optimisticRequests.map((request) => (<div className="flex items-center justify-between" key={request.id}>
            ...
            <div className="flex gap-3 justify-end">
                <form action={() => accept(request.id, request.sender.id)}><button>
                    <Image src="/accept.png" alt="" width={20} height={20} className="cursor-pointer"/>
                </button></form>
                <form action={() => decline(request.id, request.sender.id)}><button>
